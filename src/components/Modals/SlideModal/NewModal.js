...
class NewModal extends React.Component<Props, *> {
  ...
  static show(modalOptions: Object) {
    const modal = this.modalInstances[this.modalInstances.length - 1];
    if (modal) {
      modal.handleOpen(modalOptions);
    }
  }
  ...
  static isVisible() {
    const modal = this.modalInstances[this.modalInstances.length - 1];
    return modal ? modal.state.isVisible : false;
  }
  ...
  hideModal = () => {
    const { onSwipeComplete } = this.state.modalOptions;
    Keyboard.dismiss();
    ...
    const timer = setTimeout(() => {
      if (this.state.modalOptions.onModalHide) {
        this.state.modalOptions.onModalHide();
      }
      this.setState({
        isVisible: false,
      }, () => {
  ...
  handleScroll = (p: ScrollToProps) => {
    const { handleScrollTo } = this.state.modalOptions;
    if (Toast.isVisible()) {
      Toast.close();
    }
